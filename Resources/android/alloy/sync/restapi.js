...
function apiCall(_options, _callback) {
    if (Ti.Network.online) {
        ...
        xhr.onload = function() {
            ...
            _callback({
                success: success,
                status: success ? 200 == this.status ? "ok" : this.status : "error",
                code: this.status,
                data: error,
...
function Sync(method, model, opts) {
    ...
    "function" == typeof model.config.URLPARAMS ? _.extend(params.urlparams, model.config.URLPARAMS()) : "undefined" != typeof model.config.URLPARAMS && _.extend(params.urlparams, model.config.URLPARAMS);
